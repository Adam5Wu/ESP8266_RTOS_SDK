...
static const char* TAG = "boot_util";
...
static void unpack_load_app(const esp_image_metadata_t* data)
{
    bootloader_utility_start_image(data->start_addr, data->image_len, data->image.entry_addr);
}
...
void bootloader_utility_fast_boot_image(void)
{
    ...
    if (esp_fast_boot_get_info(&image_start, &image_size, &image_entry) == 0) {
...
static void enumerate_partitions_for_boot(const esp_partition_info_t *partitions, int num_partitions, void *arg)
{
    bootloader_state_t *bs = (bootloader_state_t *)arg;
    
    ESP_LOGI(TAG, "Partition Table:");
    ESP_LOGI(TAG, "## Label            Usage          Type ST Offset   Length");

    for(int i = 0, part_count = 0; part_count < num_partitions; i++) {
#ifdef CONFIG_IDF_TARGET_ESP32
        ...
        const char *partition_usage = "Unknown";
        ...
        ESP_LOGI(TAG, "%2d %-16s %-16s %02x %02x %08x %08x", part_count, partition->label, partition_usage,
...
bool bootloader_utility_load_partition_table_for_boot(bootloader_state_t* bs) {
    ...
    if (bootloader_utility_load_partition_table_for(enumerate_partitions_for_boot, bs)) {
#ifdef CONFIG_ESP8266_OTA_FROM_OLD
...
{
    if (partition->size == 0) {
        ESP_LOGE(TAG, "Can't boot from zero-length partition");
        return false;
    }
